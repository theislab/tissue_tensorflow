...
from tissue.models.layers import GCNLayer
...
class ModelGcn:
    ...
    def __init__(
        ...
        input_cluster = tf.keras.layers.Input(
            shape=(input_shapes[0], input_shapes[4]),
            dtype="float32",
            name='input_cluster'
        )
        output = []
        ...
        if aggregation.lower() == "diffpool":
            # Forward pass through node-pool-wise GCN:
            x_pool = GCNLayer(
                output_dim=aggregation_n_clusters,
                dropout_rate=dropout_rate,
                activation=None,
                l2_reg=l2_reg,
                name='Layer_gcn_diffpool',
                use_bias=True
            )([x, a])
            x, a = DiffPool(
                activation=activation,
        ...
        for d in range(aggregation_depth):
            x = GCNLayer(
                ...
                activation=activation,
                l2_reg=l2_reg,
                name='Layer_gcn_after_aggregation_' + str(d),
                use_bias=True
            )([x, a])
        ...
        for feature_name, feature_len in features.items():
            ...
            if dt == 'percentage':
                ...
                raise ValueError('Label data type not recognized: Use "categorical", "continuous", "percentage" or "survival".')
            x_out = tf.keras.layers.Dense(
                feature_len,
                activation=act,
                use_bias=True,
