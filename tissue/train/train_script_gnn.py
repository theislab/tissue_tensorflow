...
cell_type_coarseness = sys.argv[34].lower()

try:
    dispersion = sys.argv[35].lower()
    dispersion = True
except:
    dispersion = False
            ...
            for width_key in width_keys.split("+"):
                for loss_weight_others_key in loss_weight_others_keys.split("+"):
                    ...
                    if 'gcn' in model_class:
                        ...
                        kwargs_init = {"width": width_dict[width_key]}
                    elif 'reg' in model_class and dispersion:
                        kwargs_init = {"width": width_dict[width_key]}
                    ...
                    for i in range(ncv):
                        ...
                        elif 'reg' in model_class:
                            trainer = tissue.train.TrainModelREG()
                        else:
                            raise ValueError("model class %s not recognized" % model_class)
                        if dispersion:
                            trainer.init_estim(dispersion=dispersion)
                        else:
                            trainer.init_estim()
